...
public class AnimUtil {
    ...
    public void animIn(View view,Integer pivotX,Integer pivotY){
        ViewAnimator.animate(view).pivotX(pivotX).pivotY(pivotY).scale(0.1f,1).duration(300).start();
    }

    public void animOut(View view, Integer pivotX, Integer pivotY, final OnFinishListener onFinishListener){
        ViewAnimator.animate(view).pivotX(pivotX).pivotY(pivotY).duration(300).custom(new AnimationListener.Update() {
            @Override
            public void update(View view, float value) {
                ...
                if(value<=0.2f){
                    onFinishListener.onFinish(value);
                }
            }
        },1,0.1f).start();
    }
